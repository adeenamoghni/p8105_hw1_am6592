...
The pegnuins dataset is sorted described by **`r ncol(penguins_df)`** variables, which are: **`r colnames(penguins_df)`**. This dataset is **`r nrow(penguins_df)`** rows by **`r ncol(penguins_df)`** columns. The average flipper length of the penguins in this dataset is **`r mean(penguins_df[["flipper_length_mm"]], na.rm = TRUE)`** mm.
